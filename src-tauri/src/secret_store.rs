...
impl WindowsCredentialSecretStore {
    fn entry(secret_ref: &str) -> Result<Entry, String> {
        if secret_ref.is_empty() || secret_ref.len() > 200 || !secret_ref.chars().all(|c| c.is_ascii_alphanumeric() || matches!(c, '-' | '_' | '.' | '/')) {
...
impl SecretStore for WindowsCredentialSecretStore {
    fn save_secret(&self, secret_ref: &str, value: &str) -> Result<(), String> {
        if value.trim().is_empty() { return Err("密钥不能为空。".to_owned()); }
        Self::entry(secret_ref)?.set_password(value).map_err(|error| format!("无法保存到 Windows 凭据管理器：{error}"))
    }

    fn get_secret(&self, secret_ref: &str) -> Result<String, String> {
        Self::entry(secret_ref)?.get_password().map_err(|error| format!("无法从 Windows 凭据管理器读取密钥：{error}"))
    }
...
impl SecretStore for FakeSecretStore {
    fn save_secret(&self, secret_ref: &str, value: &str) -> Result<(), String> { self.0.lock().unwrap().insert(secret_ref.to_owned(), value.to_owned()); Ok(()) }
    fn get_secret(&self, secret_ref: &str) -> Result<String, String> { self.0.lock().unwrap().get(secret_ref).cloned().ok_or_else(|| "missing".to_owned()) }
    fn delete_secret(&self, secret_ref: &str) -> Result<(), String> { self.0.lock().unwrap().remove(secret_ref); Ok(()) }
    fn has_secret(&self, secret_ref: &str) -> Result<bool, String> { Ok(self.0.lock().unwrap().contains_key(secret_ref)) }
}
