...
export const useFileStore = create<FileStore>((set) => ({
  ...
  loadFiles: async (projectId) => { if (typeof window !== "undefined" && !("__TAURI_INTERNALS__" in window)) { set({ projectId, files: [], isLoading: false, error: null }); return; } set({ isLoading: true, error: null }); try { set({ projectId, files: await fileService.list(projectId), isLoading: false }); } catch (error) { set({ error: toAppError(error, "无法加载项目文件。"), isLoading: false }); } },
