...
/** @deprecated UI-only Phase 1 projection. Persisted entities live in domain.ts. */
export type ThesisProjectDashboard = { title: string; school: string; college: string; major: string; cohort: string; advisor: string; createdAt: string; currentStage: string; completion: number; metrics: { bodyWords: Metric; references: Metric; foreignReferences: Metric; journalReferences: Metric; advisorGuidance: Metric }; requirements: RequirementMetric[]; workflow: WorkflowStage[]; milestones: Milestone[]; guidanceRecords: GuidanceRecord[]; todos: Todo[]; evaluation: { score: number; dimensions: { subject: string; score: number }[] }; recentFiles: RecentFile[]; calendar: { label: string; weekdays: string[]; days: number[]; highlightedDay: number } };
