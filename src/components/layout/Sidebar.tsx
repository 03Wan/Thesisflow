...
const navigation: NavGroup[] = [
  {
    label: "准备",
    items: [
      { label: "项目总览", to: "/overview", icon: LayoutDashboard },
      { label: "项目", to: "/projects", icon: FolderArchive },
      { label: "论文要求", to: "/requirements", icon: ShieldCheck },
      { label: "选题", to: "/topic", icon: NotebookPen },
      { label: "任务书", to: "/task-book", icon: ClipboardCheck },
    ],
  },
  {
    label: "开题与研究",
  ...
  },
  {
    label: "答辩",
    items: [
      { label: "答辩准备", to: "/defense-prep", icon: Presentation },
      { label: "模拟答辩", to: "/mock-defense", icon: GraduationCap },
      { label: "答辩记录", to: "/defense", icon: ClipboardCheck },
      { label: "答辩后修改", to: "/post-defense-revision", icon: PencilLine },
    ],
  },
  {
    label: "完成",
  ...
  return (
    <aside className="sidebar">
      ...
      <NavLink
        to="/settings"
        className={({ isActive }) =>
          cn("nav-item settings-link", isActive && "nav-item-active")
        }
      >
        <Settings size={15} />
        <span>设置</span>
