...
export function SettingsPage() {
  ...
  return <section className="settings-page">
    <header className="settings-header">
      ...
      <button className="settings-reset" onClick={() => { setToggles({ autoSave: true, dueReminder: true, compactMode: false, localAnalytics: false }); setNotice("已恢复本地默认偏好"); }}><RotateCcw size={14} />恢复默认</button>
    </header>
    ...
    <div className="settings-layout">
      ...
      <main className="settings-content">
        ...
        {active === "notifications" && <SettingsGroup title="通知与提醒" description="当前仅提供应用内提醒，不会发送系统通知或邮件。">
        ...
        {active === "privacy" && <SettingsGroup title="数据与隐私" description="ThesisFlow Desktop Alpha 当前不接入账户、云端或外部学校系统。">
          <SettingRow icon={<Database size={16} />} title="数据存储状态" description="项目数据保存在当前设备；没有记录时显示为空"><span className="settings-value success">仅本地真实数据</span></SettingRow>
          ...
          <SettingRow icon={<Database size={16} />} title="清除界面偏好" description="恢复当前设备的界面开关默认值"><button className="settings-text-button" onClick={() => { setToggles({ autoSave: true, dueReminder: true, compactMode: false, localAnalytics: false }); setNotice("界面偏好已恢复默认值"); }}>恢复默认</button></SettingRow>
        </SettingsGroup>}
...
function ProviderCard({ provider, state, onChange }: { provider: ProviderDefinition; state?: ProviderState; onChange: (state: ProviderState) => void }) {
  ...
  useEffect(() => { let alive = true; new TauriSecretConfigurationStore().hasSecret(providerKey(provider.id)).then(configured => { if (!alive || state) return; const stored = readBrowserProviderConfigs().find((item) => item.id === provider.id); if (configured && !stored) saveBrowserProviderConfig({ id: provider.id, name: provider.name, protocol: provider.protocol, secret: "", model: "", baseUrl: provider.baseUrl, enabled: true }); update({ configured, enabled: stored?.enabled ?? configured, model: stored?.model ?? "", baseUrl: stored && stored.baseUrl !== provider.baseUrl ? stored.baseUrl : "", status: configured ? "待测试" : "未配置" }); }).catch(() => { const stored = readBrowserProviderConfigs().find((item) => item.id === provider.id); if (alive && !state && stored?.secret) { setSavedKey(stored.secret); update({ configured: true, enabled: stored.enabled, model: stored.model, baseUrl: stored.baseUrl === provider.baseUrl ? "" : stored.baseUrl, status: "浏览器本地已配置" }); } else if (alive && !state) update({ status: "浏览器预览：输入密钥后可测试" }); }); return () => { alive = false; }; }, [provider.id]);
  const save = async () => { if (!key) return; setBusy(true); try { await new TauriSecretConfigurationStore().saveSecret(providerKey(provider.id), key); saveBrowserProviderConfig({ id: provider.id, name: provider.name, protocol: provider.protocol, secret: "", model: current.model, baseUrl: effectiveBaseUrl, enabled: true }); update({ configured: true, enabled: true, status: "待测试", tested: "—" }); setKey(""); } catch { saveBrowserProviderConfig({ id: provider.id, name: provider.name, protocol: provider.protocol, secret: key, model: current.model, baseUrl: effectiveBaseUrl, enabled: true }); setSavedKey(key); update({ configured: true, enabled: true, status: "浏览器本地已配置", tested: "—" }); setKey(""); } finally { setBusy(false); } };
