...
export function ProposalDesignPage({ mode }: { mode: "proposal" | "design" }) {
  const isProposal = mode === "proposal"; const [editing, setEditing] = useState<string | null>(null); const [gates, setGates] = useState([true, true, true, false, false]); const [applied, setApplied] = useState(false); const [preview, setPreview] = useState(false); const [submitted, setSubmitted] = useState(false); const cards = isProposal ? proposalCards : designCards; const ready = gates.every(Boolean);
  return <section className="proposal-workspace"><header className="proposal-hero"><div><p>{isProposal ? "开题阶段 / 开题报告" : "开题阶段 / 研究设计"}</p><h1>数字经济对企业创新的影响研究</h1><div className="proposal-meta"><span className="status-chip">{submitted ? "已提交" : ready ? "可提交" : "待完善"}</span><span>版本 V1.3</span><span><CheckCircle2 size={13} />已自动保存</span><span>实证研究</span></div></div><div className="proposal-actions">{isProposal && <button className="proposal-secondary" onClick={() => setPreview(!preview)}><Eye size={15} />{preview ? "关闭预览" : "预览开题报告"}</button>}<button className="proposal-primary" onClick={() => setSubmitted(true)} disabled={!ready || submitted} title={ready ? "提交审核" : "请先完成 Stage Gate 中的未满足条件"}><Send size={15} />{submitted ? "已提交审核" : isProposal ? "提交审核" : "确认研究设计"}</button></div></header>{preview && <section className="proposal-preview"><header><b>开题报告预览</b><span>V1.3 · 本地草稿</span></header>{cards.slice(0,4).map(card => <p key={card.title}><b>{card.title}</b><span>{card.body}</span></p>)}</section>}{isProposal && <nav className="proposal-stepper">{["选题背景", "文献综述", "研究设计", "进度计划", "导师审核"].map((step, index) => <div className={index < 3 ? "done" : index === 3 ? "active" : ""} key={step}><span>{index < 3 ? <CheckCircle2 size={14} /> : index + 1}</span><b>{step}</b>{index < 4 && <i />}</div>)}</nav>}<div className="proposal-layout"><main><div className="content-heading"><div><h2>{isProposal ? "开题报告内容" : "研究设计模块"}</h2><span>以内容卡组织，点击编辑可继续完善</span></div><button className="proposal-secondary" onClick={() => setApplied(true)}><Sparkles size={14} />{applied ? "建议已应用" : "AI 补全建议"}</button></div><div className="proposal-cards">{cards.map((card, index) => <article className={editing === card.title ? "editing" : ""} key={card.title}><header><div><span>{String(index + 1).padStart(2, "0")}</span><h3>{card.title}</h3></div><div><Status value={card.state} /><button onClick={() => setEditing(editing === card.title ? null : card.title)}>{editing === card.title ? "完成编辑" : <><Pencil size={13} />编辑</>}</button></div></header>{editing === card.title ? <textarea aria-label={`编辑${card.title}`} defaultValue={card.body} autoFocus /> : <p>{card.body}</p>}<footer><FileText size={13} />{card.meta}</footer></article>)}</div></main><aside className="proposal-aside"><header><div>{isProposal ? <FileCheck2 size={16} /> : <FlaskConical size={16} />}<b>{isProposal ? "导师审核" : "AI Reviewer"}</b></div><span>{submitted ? "已提交" : ready ? "通过" : "待完善"}</span></header>{isProposal ? <><section className="gate-card"><h3>通过条件</h3>{gateItems.map((item, index) => <button className={gates[index] ? "met" : "missing"} onClick={() => setGates(items => items.map((value, itemIndex) => itemIndex === index ? !value : value))} key={item}>{gates[index] ? <CheckCircle2 size={15} /> : <CircleDashed size={15} />}<span>{item}</span><b>{gates[index] ? "已满足" : "未满足"}</b></button>)}</section></> : <section className="reviewer-card"><h3>设计评审</h3>{[["可行性", "良好", "green"], ["数据可得性", "中等", "amber"], ["模型复杂度", "适中", "blue"], ["变量可测量性", "需核验", "red"]].map(([label, value, tone]) => <p key={label}><span>{label}</span><b className={tone}>{value}</b></p>)}</section>}<section className="ai-advice"><h3>AI 建议</h3><p>建议补充数字基础设施影响资源配置效率的机制文献，并将变量定义与模型公式逐一对应。</p><button onClick={() => setApplied(true)}>{applied ? <><CheckCircle2 size={14} />已应用建议</> : <><Sparkles size={14} />应用到文档</>}</button></section>{!ready && <div className="gate-warning"><LockKeyhole size={15} /><span>Stage Gate 尚有 {gates.filter(value => !value).length} 项未满足，提交已禁用。</span></div>}</aside></div></section>;
}
